...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        cb(null, 'tmprt/');//profile tmp image 
    },
...
async function returnImageMiddleware(req, res, next) {


    //console.log(" return file middlewre in admin calling");
    ...
    upload(req, res, async (err) => {
     ...
     //console.log(pathIm2);
        ...
        const f1name = `rtIm/${Date.now()}-${Math.round(Math.random() * 100000)}.jpg`;
        ...
        //resize image 2
        const f2name = `rtIm/${Date.now()}-${Math.round(Math.random() * 100000)}.jpg`;
        ...
        //file handling done 
        //attach req,body
        req.body.retImage=f1name;//image1 for retrun image
        req.body.billImage=f2name;//image2 is bill of payment 
...
module.exports = returnImageMiddleware;

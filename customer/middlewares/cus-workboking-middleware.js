...
const upload = multer({ storage: storage }).single('workimage');// 

function customerworkingbookingMiddleware(req, res, next) {
    ...
    upload(req, res, async (err) => {
        ...
        const fname = `wrkIm/${Date.now()}-${Math.round(Math.random() * 100000)}.jpg`;
...
module.exports = customerworkingbookingMiddleware;
